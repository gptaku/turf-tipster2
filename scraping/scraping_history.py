...
scrap_year = [2016,2017,2018,2019,2020]
num_process = 1
...
with open('horse_id-keibalab.txt') as idfile:
...
#	s = eval('b"7b\7d"'.format(str(meta))).decode('EUC_JP')
	wp = 5
...
def scrap_one_horse(url):
	...
	try:
		...
		req = urllib.request.Request(url_base_keibalab+'/db/horse/'+url+'/', data, headers)
		time.sleep(0.3)
				...
				for yy in scrap_year:
					...
					for mm in range(1,13):
						...
						if horse_name!= '' and len(meta) == 218:
							...
							print('print1: '+len(horse_meta),len(hist_meta),len(each_rank),len(rank_val),len(meta))
...
def scrap_one_horse2(url):
	time.sleep(1)
	
	print('test')
	
	return "pid: %d," % (os.getpid())
#	return "pid: %d, scrap_one_horse2: %s," % (os.getpid(), url)

proc_pool = Pool(num_process)
print('Start')
#result = proc_pool.map(scrap_one_horse, horse_urls)
result = proc_pool.imap_unordered(scrap_one_horse, horse_urls)
#result = proc_pool.map_async(scrap_one_horse, horse_urls)
